...
| E-17 | operator | P-01 / P-02 / P-12 / P-11 / P-03 | `tick`; `status`; `explain`; `anchor`; `decide`; `pending` (P-11 `pending()`); `onboard` | CLI commands: one sweep; FR-016 disposition and reason; FR-012 reconstruction; an anchored record head; a named decision with basis; a starter config | the CLI on the operator machine; the only human interface |
