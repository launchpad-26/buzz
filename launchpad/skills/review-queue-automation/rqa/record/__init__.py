...
public surface is §1's re-export list plus the one concrete collaborator a
composition root must construct: `SQLiteRecordWriter` (the one implementation of the
`RecordWriter` seam every other part is handed). `SQLiteRecordReader` stays a module
name (`rqa.record.reader`).

**Why that one is surface.** Every part that appends takes its `RecordWriter` as a
