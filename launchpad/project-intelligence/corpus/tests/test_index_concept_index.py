"""Unit tests for the concept-index builder (index_defs/concept_index.py) --
issue #889.
...
change what those tests assert. The committed generated/concept-index.md is
touched read-only, and only for properties the builder's SPEC fixes (id, type,
marker) -- never for digest- or listing-dependent content that shifts whenever
any corpus node changes.
"""
...
TEMPLATE_ID = "corpus-template-concept"
EMPTY_SENTENCE = "so the concept index is"


def _concept_index_spec() -> "indexes.IndexSpec":
    specs = [
        s for s in indexes.discover_builders(REAL_DEFS_DIR) if s.name == "concept-index"
    ]
    assert specs, "concept-index builder not discovered"
    return specs[0]
...
def _write_node(root: Path, rel: str, node_id: str, implements: str | None) -> None:
    lines = [
        ...
        "  - agent",
        "evidence:",
        '  - statement: "Fixture node for test_index_concept_index.py."',
        "    entry_class: FACT",
        "    evidence:",
...
class DiscoveryTest(unittest.TestCase):
    def test_concept_index_builder_discovered_with_declared_identity(self) -> None:
        spec = _concept_index_spec()
        self.assertEqual(spec.output_path, "generated/concept-index.md")
        self.assertEqual(spec.node_id, "generated-concept-index")
...
class InclusionRuleTest(unittest.TestCase):
    ...
    def test_node_implementing_the_template_is_listed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            _write_node(root, "concept-template.md", TEMPLATE_ID, None)
            _write_node(root, "concept-huddle.md", "concept-huddle", TEMPLATE_ID)
            _write_node(root, "concept-relay.md", "concept-relay", TEMPLATE_ID)
            _write_node(root, "unrelated.md", "unrelated-node", None)
            text = self._render(root)
        self.assertIn("| concept-huddle | `concept-huddle.md`", text)
        self.assertIn("| concept-relay | `concept-relay.md`", text)
        # Sorted by node id: concept-huddle before concept-relay.
        self.assertLess(
            text.index("| concept-huddle |"), text.index("| concept-relay |")
        )
    ...
    def test_empty_match_renders_honest_empty_listing(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _write_node(root, "unrelated.md", "unrelated-node", None)
            text = self._render(root)
        self.assertIn("## Concept nodes", text)
        self.assertIn(EMPTY_SENTENCE, text)
...
class CommittedDocumentTest(unittest.TestCase):
    ...
    def test_committed_concept_index_front_matter_and_marker(self) -> None:
        text = (REAL_CORPUS_ROOT / "generated" / "concept-index.md").read_text()
        self.assertIn('id: "generated-concept-index"', text)
