"""Tests for the specifications-index builder (index_defs/specifications_index.py)
-- issue #1302.

Follows test_index_api_index.py's conventions: indexes.py is loaded by path
...
OUTPUT_REL = "specifications/INDEX.md"
...
audiences:
  - agent
evidence:
  - statement: "This node is a fixture used only by the specifications-index builder's tests."
    entry_class: FACT
    evidence:
...
Fixture node for test_index_specifications_index.py.
"""


def _write_node(corpus_root: Path, rel_path: str, node_id: str, node_type: str) -> None:
    path = corpus_root / rel_path
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(_NODE_TEMPLATE.format(node_id=node_id, node_type=node_type))
...
class SpecificationsIndexSpecTest(unittest.TestCase):
    def _spec(self):
        by_name = {s.name: s for s in indexes.discover_builders(DEFS_DIR)}
        self.assertIn("specifications-index", by_name)
        return by_name["specifications-index"]

    def test_builder_discovered_with_declared_identity(self) -> None:
        spec = self._spec()
        self.assertEqual(spec.output_path, OUTPUT_REL)
        self.assertEqual(spec.node_id, "specifications-index")
        self.assertEqual(spec.node_type, "governance")


class SpecificationsIndexGenerationTest(unittest.TestCase):
    ...
    def test_inclusion_rule_selects_only_specifications_path_prefix(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            _write_node(
                root,
            ...
            _write_node(root, "fixture-root-node.md", "fixture-root-node", "governance")
            _write_node(
                root,
            ...
            text = _generate(root)
        self.assertIn("| `fixture-spec-node` |", text)
        self.assertNotIn("| `fixture-root-node` |", text)
    ...
    def test_front_matter_carries_node_id_and_type(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            text = _generate(root)
        front = text.split("---")[1]
        self.assertIn('id: "specifications-index"', front)
