...
# A GENERATED node records an INPUT DIGEST, not a commit (#2173). Its content
# is a pure function of the corpus it was built from, so there is no single
# commit that "authored" it -- and the commit that would be cited is the one
# that will contain it, which does not exist when it is written. Requiring a
# SHA of it is a category error, so the ladder below recognises this shape
# first and reports the digest as the node's established provenance rather
# than falling through to rung 3's "unestablished".
_GENERATED_PROVENANCE_STATEMENT_RE = re.compile(
    r"generated by\s+\S*indexes\.py\s+from\s+\d+\s+canonical corpus node\(s\);"
    r"\s+input digest\s+sha256:(?P<digest>[0-9a-f]{64})",
    re.IGNORECASE,
)
...
class RecordedRevision:
    ...
    # Set INSTEAD of `sha` for a generated node, whose provenance is the digest
    # of its inputs rather than a commit (#2173). A node with either one has
    # established provenance; a node with neither has none.
...
def extract_recorded_revision(node: "validate.LoadedNode") -> RecordedRevision:
    ...
    Rung 0, before any of them: a GENERATED node records an input digest
    rather than a commit (#2173), so it is resolved on that and never reaches
    the ladder. See `_GENERATED_PROVENANCE_STATEMENT_RE`.
    """
    for entry in node.data.get("evidence") or []:
        if not isinstance(entry, dict):
            continue
        generated = _GENERATED_PROVENANCE_STATEMENT_RE.search(
        ...
        if generated:
            return RecordedRevision(
                node.id,
                None,
                "generated node: provenance is an input digest, not a commit",
                digest=generated.group("digest"),
            )
