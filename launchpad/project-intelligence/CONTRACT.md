...
| # | Divergence | Status |
|---|---|---|
| 1 | Design doc says `knowledge.explain(symbol, depth?)` is "depth-tunable". | **Resolved — #571.** All six depths render distinguishably (§4). `RATIONALE` remains the only one that changes what was investigated; `IMPACT` delegates whole to `impact()`; the other four render one shared investigation differently. See row 8 for the one sub-bullet that shipped narrowed rather than as written. |
...
| 8 | Design doc's § Data Model item 6 says `ONBOARDING` is "Mechanism + why + key files, **no deep internals**." Implementation renders every populated section, including `Relevant flow` and `Sources`. | **Deliberate narrowing — #571, found by review-final.** `ONBOARDING` is `classify_depth()`'s own no-signal-phrase fallback, so it is what an unqualified `explain(agent, symbol)` already rendered before #571 — a pre-existing test (`test_worked_answer.py::test_all_six_sections_are_present`) pinned that default to every section. Restricting `ONBOARDING` as the design doc describes would have silently changed today's default answer shape mid-fix. Shipped instead: `ONBOARDING` renders identically to unspecified depth. |
