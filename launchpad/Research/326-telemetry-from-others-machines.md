...
5. **Reidentification applies even to a stated-identity system.** Go worried that obscure configurations identify organisations. Here, machine-specific detail — hostnames, paths under a home directory, installed tooling — identifies more than the pubkey already does. The filtering policy should treat incidental machine detail separately from the deliberate member attribution.
