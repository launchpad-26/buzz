...
**Summary:** **The `rr-cache` is portable** — copied into a fresh clone it replays, so every remedy #300 considers is viable. **The fragility is real and the window is tight**: an upstream edit far from the conflict still replays; an edit about five lines away does not. **When it declines it is loud** (normal conflict markers). **The genuinely silent hazard is different from the one on record**: upstream drift cannot cause a stale replay, because drift changes the preimage and the match simply declines — what reapplies silently is a resolution that was recorded *wrongly the first time*. This corrects my own answer on #362. And a finding that matters for automation: **`rerere` never turns a conflicted merge into a clean one** — every replay still ends "Automatic merge failed".
...
| 4b. Upstream edits a line ~5 lines from the hunk | **Does not replay** — records a new preimage, leaves markers |
...
## 4. Fragility: real, and the window is about five lines
...
**So ADR-0021's fragility claim is correct in substance and the window is tighter than "when upstream evolves the code around a declined hunk" suggests — five lines was enough.** For a file upstream is actively rewriting, a stored resolution is close to worthless.
...
On [#362](https://github.com/launchpad-26/buzz/issues/362) I wrote that ADR-0021's stated mechanism was wrong and that the real silent hazard was `rerere` "firing stale" when *upstream's intent changed underneath*. **Experiment 4b shows that specific mechanism cannot happen.** The preimage is the conflict — both sides plus context — so if upstream's side changes, the preimage changes and the match declines. Upstream drift produces a *decline*, never a stale fire. My #362 framing was wrong on the mechanism even though it was right that the record's reason needed correcting.
...
**So the honest statement for the record:** `rerere` declines loudly on drift and reapplies silently on error. Its risk is not that upstream moves — it is that the first resolution was wrong and nobody notices for as long as the cache lives. Which makes a *shared* cache higher-stakes than a per-clone one: one person's mistake propagates to everyone and to CI.
...
**ADR-0021's `rerere` reasoning needs one correction, and my #362 answer needs another.** The record says the replay "silently stops firing"; it stops firing *loudly*. I said the silent hazard was upstream drift; it is recorded-wrong resolutions. Both corrections point the same way — the record's conclusion (`rerere` is a labour-saver, not a durability mechanism) is right — but the fork should not carry two wrong mechanisms in support of a right conclusion.
...
**The five-line fragility window argues against `rerere` for exactly the files the fork cares about most.** `lefthook.yml` is a file upstream rewrites; `runtime.rs` is a file upstream restructures. Those are the two rows where a stored resolution is least likely to survive and most likely to be wrong if it does. #294's mechanism column should probably not assign `rerere` to either.
...
the real horizon, and I withdraw it.** It rested on the five-line fragility window mattering — on
...
**High.** Every claim is an experiment with pasted output, run against the real three versions of `lefthook.yml` rather than a toy, and the decisive ones (portability, adjacent-edit decline, silent wrong replay) were each observed directly.

**Not verified.** I tested one file and one conflict shape. `rerere`'s documented behaviour with *multiple* conflicts in one file — where practitioner accounts say it becomes stricter and may re-ask for the whole set — was not tested, and the fork's `runtime.rs` conflict is the case that would exercise it. I did not test the `conflict-marker-size` interaction that `git-rerere(1)` warns about for files containing marker-like lines. I measured the fragility window at two distances, ~70 lines and ~5 lines; **I did not bisect it**, so "about five lines" is an upper bound on what still fails, not a measured threshold — the real boundary is somewhere between 5 and 70 and is presumably git's diff context, which I did not confirm. I did not test a cache shared through `actions/cache` or a git branch, only a direct filesystem copy — that is the mechanism those transports reduce to, but I have not run the transports themselves. I did not test whether a `postimage` recorded on one platform replays on another. Nothing here was run in CI. I did not run `just ci`; the only repository change is one markdown file.
