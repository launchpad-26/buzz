...
| 5 | Library health sweep (duplicates, orphans, broken links, contradicting claims, missing provenance records) | 3 | Not started |
...
independently) — it is one skill with three modes (`bootstrap`/`place`/`sweep`), not three skills
— **corrected 2026-09-05: `sweep` growing to five checks (§6.6) added checks within that one mode,
not a fourth mode** — and **(decided 2026-09-04, by Serina) every gate in this diagram runs twice, not
...
| **Contradiction detection — how it stays bounded** | Decided 2026-09-04, by Serina. Naive cross-page contradiction checking is all-pairs over the whole library — unbounded, and `verify-claims` (§6.7) already declined that shape for exactly this reason. `sweep` instead groups claims by **`{repo, path, commit}` plus overlapping `span`** (corrected 2026-09-05 — grouping by path/span alone would ignore `repo`/`commit`, letting two claims about the same path at different commits, or different repos, be compared as if they cited the same code) — every claim across the library whose citation matches on all of those (using `provenance-log`'s own per-section records, §8, rather than re-deriving citations from scratch) lands in the same group — and only dispatches a comparison *within* a group, never across unrelated ones. Two claims about different code, or the same file at different commits, can't contradict each other in any way this check is built to catch; two claims about the *same* code at the *same* commit with overlapping spans are exactly where drift or disagreement is both likely and cheap to compare, since the group size is bounded by how many pages cite that one span, not by the library's total size. |
...
Two representations of the same fact, each with exactly one owner so they don't drift —
**not written atomically, corrected 2026-09-05: "written together" read as a stronger guarantee
than intended.** `draft-page`/`update-page`'s own text (§6.2/§6.3) is explicit that the page write
and the ledger append are ordered, not transactional — a crash between the two is possible on a
plain filesystem, and this design accepts that rather than building a rollback protocol, relying
on the ordering (page first, ledger after) to make a partial failure safe to retry rather than
silently wrong:
...
**Delivers:** `scan-repo`'s complete procedure (`new`/`stale`/`removed`/`needs_baseline` —
corrected 2026-09-05 to name all four lists, not just the original three) run for real against the
...
hand — its `new` list automatically driving `draft-page` per entry, its `needs_baseline` list
automatically driving `draft-page` in baseline mode, its `removed` list automatically handed to
`library-index` `sweep` mode for reconciliation.
**Depends on:** Phase 2.
...
the tool) across **all four** lists, not just `new` — **including at least one genuinely
`unknown-pre-existing` section from a `library-index bootstrap` adoption, confirming it reports on
`needs_baseline` and that `draft-page`'s baseline mode actually resolves it** (added 2026-09-05, a
review found this phase's own review gate never exercised the path this document requires) —
deliberately include, in the test repo's
