...
**Summary:** Two paths, and only one can be scoped. Direct access to Loki, Prometheus and Tempo carries no authentication at all — Loki's own guidance is that you run an authenticating reverse proxy in front. Through Grafana, one service account token reaches all three signals, and the Viewer role is genuinely read-only. So criterion 4's credential has a clean answer — one Grafana service account token with the Viewer role — and the agent must not be given direct store access, because that cannot be restricted.
...
2. **#289's own security note is answered better than it feared.** It warns that *"the agent's query path is a credential… read access to the cohort's full telemetry picture, held by an automated process"*. Read access is exactly what Viewer grants, and nothing more — this is a solved problem, provided the agent goes through Grafana.
