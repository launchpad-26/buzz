...
- **Bounded unknown** — the component boundary and assessment owner are verified, but its
  component-level runtime instrumentation/export behavior has not yet been established.
...
relay route and WebSocket family, Tauri command/event/channel family, ACP/MCP method
family, CLI family, registered startup/background task, first-party dependency adapter,
subprocess edge, and product signal/export surface found at the pinned revision maps to a
named ID above. Mobile, mobile push, test/build/example processes, dependency internals,
and observability infrastructure are examined exclusions X01-X05. There is no anonymous
or unexamined in-scope bucket in this stage-1 record.
