...
own — no generated view exists that would surface it, and no check reads it.
...
missing edges.** One `evidence` entry cites `launchpad/docs/corpus/AGENTS.md` by path,
and a bare path is checked against the filesystem. If this node reached `launchpad` ahead
of the branch introducing that file, that citation would fail exactly as an unmatched
...
- **Whether any reader ever sees `status` is unknown.** The claim that a retired node
  ...
  What is verified is narrower: no *check* reads the field.
