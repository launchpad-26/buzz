...
evidence:
  ...
  - statement: "When a NIP-GS envelope carries an optional NIP-OA owner-attestation (`oa` field), the spec requires a verifier to treat 'signed by the agent key' and 'authorized by the owner key' as two distinct facts, and states that if the owner-attestation signature fails verification the outer commit signature (`sig`) may still be valid -- a verifier MUST report the commit as signed but the owner authorization as failed or unverified, never collapse the two into one pass/fail result."
    entry_class: FACT
    evidence:
      - "docs/nips/NIP-GS.md:395-426"
