...
evidence:
  ...
  - statement: "`get_member_role` and `is_member` in `crates/buzz-db/src/store/channel_members.rs` read a pubkey's channel-scoped role (or membership) from `channel_members`, scoped by `community_id`, `channel_id` and `pubkey`, and excluding rows where `removed_at IS NOT NULL` — a channel-local, not community-wide, membership/role read."
    entry_class: FACT
    evidence:
...
- **Channel-scoped reads.** `get_member_role`/`is_member`
  (`crates/buzz-db/src/store/channel_members.rs`) are how a handler learns a pubkey's role
