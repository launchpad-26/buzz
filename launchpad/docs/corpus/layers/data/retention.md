...
evidence:
  ...
  - statement: "Every Redis-backed mechanism inspected in this codebase (presence, rate limiting, cache invalidation, NIP-98 replay guard) writes keys with an explicit TTL or EXPIRE, and buzz-pubsub's own module doc comment describes Redis's role as pub/sub fan-out, presence, and typing indicators layered on top of PostgreSQL as the durable store -- consistent with Redis holding no data this repository treats as a system of record. This was checked against every TTL/EXPIRE call site found in crates/buzz-pubsub and crates/buzz-auth, not against every Redis key ever written in the repository, so it is reported as a reasoned pattern rather than an exhaustive inventory."
    entry_class: INFERENCE
    evidence:
...
| **Postgres — two named high-churn kinds** | None. `kind:30078` NIP-RS read-state markers and `kind:30003` mesh-status heartbeats are the sole exception to the tombstone default: a database trigger issues a real `DELETE` the instant either is superseded or soft-deleted, because — in each migration's own words — a NIP-RS payload "has no historical product value" and only a mesh-status heartbeat's live head does. | The trigger itself, on every write, for these two kinds only. This is a hardcoded exception list, not a general TTL policy any new high-churn kind gets automatically. |
| **Postgres — ephemeral channels** | Bounded by the channel's own `ttl_seconds`, refreshed forward on every new durable event written into it. A permanent channel (`ttl_seconds` unset) has no bound at all. | A background reaper archives (`archived_at`) an ephemeral channel once its deadline passes. Archiving is not deletion — the channel container and the events inside it are untouched by this mechanism; only the container stops being active. |
...
| **Redis** | Bounded by construction, key by key — nothing observed is written without an expiry. Presence keys carry a 180-second TTL (three times the heartbeat interval); rate-limit keys are `INCR`'d with a conditionally-applied `EXPIRE`, repaired if a crash left one missing; cross-pod cache-invalidation messages exist only as a fallback to a 10-second local-cache TTL; NIP-98 replay-guard keys carry a 120-second floor. | The key's own TTL, in the ordinary case. A whole-community deletion's `CachePurged` stage additionally `SCAN`s and `UNLINK`s the entire `buzz:{community}:*` namespace, so a community's Redis footprint is not left to expire key-by-key on deletion. |
...
Postgres tombstones and Redis TTLs each bound what a normal read or a normal
key lookup can see, but the underlying bytes persist — sometimes literally
