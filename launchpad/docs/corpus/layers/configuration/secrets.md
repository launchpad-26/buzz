...
evidence:
  ...
  - statement: "BUZZ_PRIVATE_KEY is the required identity variable for buzz-acp and the buzz CLI (documented in .env.example's ACP section as 'REQUIRED — identifies the agent on the relay'); crates/buzz-test-client/src/main.rs reads it via std::env::var and calls .expect('invalid BUZZ_PRIVATE_KEY') on parse failure; crates/buzz-agent/src/mcp.rs's PASSTHROUGH_ENV allowlist (used in spawn_one's env_clear()-then-passthrough loop) actually names NOSTR_PRIVATE_KEY alongside BUZZ_PRIVATE_KEY, BUZZ_RELAY_URL and BUZZ_AUTH_TAG — it is not excluded from the list. The code's own comment states dev-mcp writes NOSTR_PRIVATE_KEY to a keyfile and removes it from its own process environment before any subprocess spawns, so std::env::var(\"NOSTR_PRIVATE_KEY\") returns Err and nothing is passed through in practice, even though the name remains on the allowlist."
    entry_class: FACT
    evidence:
      - ".env.example"
...
| `BUZZ_PRIVATE_KEY` | Nostr private key (hex or `nsec1…`) | none | yes, for any ACP-harnessed agent or CLI operation that signs | yes | Identifies the agent process on the relay; `buzz-test-client` panics on an invalid value, and it is passed through into spawned MCP subprocess environments via `mcp.rs`'s `PASSTHROUGH_ENV` allowlist. That allowlist also names `NOSTR_PRIVATE_KEY`, but dev-mcp clears that variable from its own process environment (after writing it to a keyfile) before any subprocess spawns, so its value is never actually present to pass through despite the name remaining on the list. |
