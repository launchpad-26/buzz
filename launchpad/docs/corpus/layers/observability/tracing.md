...
evidence:
  ...
  - statement: "The literal string `#[instrument]` (unqualified, no crate prefix) appears exactly once in the workspace's own .rs source, and it is documentation text, not a real attribute: it sits inside crates/buzz-relay/src/telemetry.rs's module-level `//!` doc-comment ASCII diagram, illustrating that the tracing pipeline's inputs are \"spans + events from #[instrument] and macros.\" Grepping for the crate-qualified form actually written on real functions, `#[tracing::instrument` or `#[::tracing::instrument`, finds five occurrences across four files: crates/buzz-relay/src/handlers/event.rs (two: `fan_out_pubsub_event` and `handle_event`), crates/buzz-relay/src/handlers/auth.rs (`handle_auth`), crates/buzz-relay/src/subscription.rs (`fan_out_scoped`, a synchronous, non-async function), and the one generated inside crates/buzz-datastore-tracing/src/lib.rs's `datastore_span` macro expansion."
    entry_class: FACT
    evidence:
      - "crates/buzz-relay/src/telemetry.rs:1-11"
      - "crates/buzz-relay/src/handlers/event.rs:281"
      - "crates/buzz-relay/src/subscription.rs:378-379"
...
**1. Function-level `#[instrument]` attribute.** The crate-qualified `#[tracing::instrument`
attribute is written on real functions five times across four files: twice in
`crates/buzz-relay/src/handlers/event.rs` (`fan_out_pubsub_event`, `handle_event`), once in
`crates/buzz-relay/src/handlers/auth.rs` (`handle_auth`), once in
`crates/buzz-relay/src/subscription.rs` (`fan_out_scoped` — notably a synchronous, non-async
function, showing `#[instrument]` is not limited to `async fn`), and once generated by the
`datastore_span` proc macro (the code-generated variant described after shape 4 below). The
simplest form, `#[tracing::instrument(skip_all)]`
on `fan_out_pubsub_event` and `fan_out_scoped`, declares no fields at all. `handle_event` and
`handle_auth` go further: `#[tracing::instrument(skip_all, fields(event_id, kind))]`-shaped
attributes declare named fields with no assigned value up front, and the function body later
calls `tracing::Span::current().record("event_id", ...)` once the value becomes available. This
...
once they're known, rather than requiring them at creation time. (The unqualified literal
`#[instrument]` also appears once in this workspace, but only as illustrative text inside
`crates/buzz-relay/src/telemetry.rs`'s module doc-comment diagram — not a real attribute.)
...
**A code-generated variant reuses shape 1.** The `buzz-datastore-tracing`
