...
**What to do with it.** Find your area in the tables. `NOT_EVALUATED` means I did not look —
...
`INCORRECT` / `STALE` / `UNKNOWN` / `NOT_EVALUATED`. `NOT_EVALUATED` is not a pass.
...
| `launchpad/ARCHITECTURE.md` | System architecture for the cohort's operation | Developer, operator | ARCH-001, ARCH-005 | ARCH-006 data flow; ARCH-009 background jobs | `NOT_EVALUATED` | Medium | Assess in the first deep-review tranche |
...
| `launchpad/REQUIREMENTS.md` | Cohort requirements | All | FOUND-003 | — | `NOT_EVALUATED` | Medium | Assess |
| `launchpad/ENVIRONMENTS.md` | Environment inventory | Operator | START-005, ARCH-008 | — | `NOT_EVALUATED` | **High** — environment facts move fastest | Assess early |
...
| `templates/` | 26 | Genre contracts per node type | Agent | R02 genre overlays; `API-012` | — | `NOT_EVALUATED` | Low — templates change slowly | Assess against `R02`'s eight families |
| `architecture/` | 47 | Context, container, deployment, flow, principle views | Developer, operator | ARCH-001…005 | ARCH-006, ARCH-009 | `NOT_EVALUATED` — the research found (2026-09-07) 48 nodes all `draft`, 7 with diagrams, 2 with relationships; relationship coverage has since risen corpus-wide to 69% | Medium | First deep-review tranche |
| `layers/` | 170 | Cross-cutting system layers — the largest single group | Developer | ARCH-006/007, API-008 | — | `NOT_EVALUATED`; 2 of 4 random-sample claims verified here | Medium | Sample-based review |
| `capabilities/` | 140 | Product capabilities | Developer, reviewer | R02 G01 | — | `NOT_EVALUATED` | Medium | Sample-based review |
| `operations/` | 36 | Runbooks, databases, deployment, reliability | Operator | OPS-004…007 | — | `NOT_EVALUATED` — **highest-consequence unreviewed group** | **High** | **Review first** (see roadmap) |
| `verification/` | 45 | CI, test contracts, assurance | Reviewer | DEV-002/004, FOUND-011 | — | `NOT_EVALUATED`; 1 random-sample claim verified here | Medium | Sample-based review |
| `interfaces/`, `events/` | 64 | WebSocket, HTTP, event kinds | Developer | API-001…005, API-011 | — | `NOT_EVALUATED`; 1 random-sample claim verified here | **High** — protocol surfaces move | Deep review after operations |
| `platforms/`, `implementation/`, `ingestion/`, `development/`, `releases/`, `agents/`, `governance/`, `specifications/`, `decisions/` | 221 | Remaining subject surfaces | Mixed | Various | — | `NOT_EVALUATED` | Medium | Sample-based |
...
| `deploy/` (compose, configs) | 41 md + configs | Deployment configuration | Operator | API-008/009 | OPS-001/002 logging and monitoring | `NOT_EVALUATED` | **High** | Assess with operations |
...
| `skills/`, `agents/`, `review-agent/` | 185 | Agent skill and persona definitions | Agent | `NOT_EVALUATED` | Medium | Assess against section I |
| `decisions/` | 67 | ADRs | All | `NOT_EVALUATED` | Low | Check `GOV-007` supersession marking across all 67 |
