...
use crate::{
    ...
    managed_agents::{
        ...
        load_managed_agents, load_personas, portable_config_for_import, save_managed_agents,
...
pub async fn confirm_agent_snapshot_import(
    ...
    let portable_env = portable_config_for_import(
        ...
        &snapshot.definition.portable_env,
        snapshot.definition.thinking_effort.as_deref(),
    )?;
...
// Avatar-materialization tests live in a sibling file so this module stays under
// the desktop file-size ratchet.
#[cfg(test)]
