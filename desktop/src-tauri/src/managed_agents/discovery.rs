...
// Avatar URLs live with the runtime table; the discovery tests assert against
// them, so re-export for that consumer only (no non-test reader here).
#[cfg(test)]
...
fn discover_acp_runtime_phase1(runtime: &'static KnownAcpRuntime) -> PartialEntry {
    ...
    PartialEntry {
        ...
        entry: AcpRuntimeCatalogEntry {
            ...
            thinking_config_json_env_var: runtime.thinking_config_json_env_var.map(str::to_string),
            thinking_config_json_key: runtime.thinking_config_json_key.map(str::to_string),
